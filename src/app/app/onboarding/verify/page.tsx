...
export default function VerifyProfilePage() {
  ...
  const handleSkip = () => {
    // Navigate back to onboarding start
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex flex-col bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-indigo-950">
      ...
      {/* Fixed Header - Updated for better mobile responsiveness */}
      <div className="fixed top-0 z-10 w-full backdrop-blur-md bg-white/80 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
            {/* Left side with icon and title - Mobile friendly layout */}
            <div className="flex items-center space-x-2 mb-2 sm:mb-0">
              ...
              <div>
                <h1 className="text-base sm:text-lg font-bold text-black dark:text-white">Verify Your Business Profile</h1>
                <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Ask questions to ensure all information is accurate</p>
              </div>
            </div>

            {/* Step Indicator - Hidden on very small screens */}
            <div className="hidden sm:flex items-center space-x-2 lg:space-x-4 order-3 sm:order-2">
              <div className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400 font-bold text-xs">
                1
              </div>
              <div className="h-0.5 w-4 sm:w-8 bg-gray-300 dark:bg-gray-600"></div>
              <div className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400 font-bold text-xs">
                2
              </div>
              <div className="h-0.5 w-4 sm:w-8 bg-gray-300 dark:bg-gray-600"></div>
              <div className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-blue-600 text-white font-bold text-xs">
                3
              </div>
            </div>
            
            {/* Complete button - Full width on mobile, normal on desktop */}
            <button
              onClick={handleFinish}
              className="w-full sm:w-auto group relative flex items-center justify-center space-x-1 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-md hover:shadow-lg transition-all duration-200 overflow-hidden order-2 sm:order-3"
            >
      ...
      {/* Chat Container - Adjusted to account for fixed header AND fixed input */}
      <div className="flex-grow overflow-hidden flex flex-col max-w-4xl mx-auto w-full pt-24 sm:pt-28 pb-36 sm:pb-40">
        {/* Messages Area */}
            ...
            {messages.map((message) => (
              ...
              >
                <div className={`flex max-w-[85%] md:max-w-[75%] ${message.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  >
                    <p className={`text-sm leading-relaxed ${message.sender === 'user' ? 'text-white' : 'text-black dark:text-white'}`}>{message.text}</p>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-10 p-4 backdrop-blur-md bg-white/80 dark:bg-gray-800/80 border-t border-gray-200 dark:border-gray-700 shadow-t">
                  ...
                  <div className="flex space-x-2">
                    {[
                      ...
                      <button
                        ...
                        disabled={isChatDisabled}
                        className={`text-xs whitespace-nowrap px-3 py-2 bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700 text-black dark:text-white rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center transition-all duration-200 ${
